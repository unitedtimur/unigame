...
Level_Parallelogram_1::Level_Parallelogram_1(Matrix* matrix, GraphicView* view, GraphicScene* scene) :
    _matrix(matrix),
    _view(view),
    _scene(scene)
{
    ...
    connect(_view, &GraphicView::mouseClicked, this, &Level_Parallelogram_1::isInsidePolygon);
    connect(_matrix->ui->levelButton, &QPushButton::clicked, this, &Level_Parallelogram_1::clearLevel);
}

void Level_Parallelogram_1::paintPointOnGraphicView(QMouseEvent* event)
{
    double rad = 5;
...
void Level_Parallelogram_1::paintLineOnGraphicView(QMouseEvent* event)
{
...
void Level_Parallelogram_1::isInsidePolygon(QMouseEvent* event)
{
    if (event->button() == Qt::MouseButton::RightButton)
    {
...
void Level_Parallelogram_1::paintLevel()
{
    ...
    for (qint32 i = 0; i < 6; ++i)
    {
        if (i == 2) {
            ...
            points.push_back(QPoint(width * 5, height * 2));
        }

        if (i == 4) {
            points.push_back(QPoint(width * 1, height * 4));
            points.push_back(QPoint(width * 4, height * 4));
        }
    }
...
void Level_Parallelogram_1::paintPoint(const QPoint& point)
{
    double rad = 5;
...
void Level_Parallelogram_1::clearLevel()
{
    // Очищаем уровень
    _previousPos = QPoint();
    _scene->clear();
...
void Level_Parallelogram_1::showTooltip()
{
}

void Level_Parallelogram_1::showHint()
{
    _matrix->ui->hintLabel->setText(QObject::tr("Соедините четыре данные точки так,\nчтобы получился праллелограм"));
}

void Level_Parallelogram_1::startLevel()
{
...
bool Level_Parallelogram_1::checkLevel(QObject* watched, QEvent* event)
{
    Q_UNUSED(watched)
...
void Level_Parallelogram_1::finishLevel()
{
...
bool Level_Parallelogram_1::inArea(const QPoint& first, const QPoint& second, const qint32& epsilon)
{
